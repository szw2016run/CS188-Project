...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        t = nn.ReLU(nn.AddBias(xm1, self.b1))
        xm2 = nn.Linear(t, self.w2)
        return nn.AddBias(xm2, self.b2)
    ...
    def train(self, dataset):
        ...
        while (True):
            for x, y in dataset.iterate_once(1):
                gradient = nn.gradients(self.get_loss(x, y), [self.w1, self.w2, self.b1, self.b2])
...
class DigitClassificationModel(object):
    ...
    def run(self, x):
        ...
        "*** YOUR CODE HERE ***"
        xm1 = nn.Linear(x, self.w1)
        t1 = nn.ReLU(nn.AddBias(xm1, self.b1))
        xm2 = nn.Linear(t1, self.w2)
        t2 = nn.AddBias(xm2, self.b2)
        xm3 = nn.Linear(nn.ReLU(t2), self.w3)
    ...
    def train(self, dataset):
        ...
        while True:
            ...
            for x, y in dataset.iterate_once(2):
                gradient = nn.gradients(self.get_loss(x, y), [self.w1, self.w2, self.w3, self.b1, self.b2, self.b3])
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        flag = True
        for x in xs:
            layer1 = nn.AddBias(nn.Linear(x, self.w), self.b1)
            relV2 = nn.Linear(nn.ReLU(layer1), self.wHidden)
            layer2 = nn.AddBias(relV2, self.b2)

            if flag:
                result = nn.ReLU(layer2)
                flag = False

            else:
                result = nn.Add(layer2, nn.Linear(nn.ReLU(result), self.wHidden))

        fin = nn.Linear(nn.ReLU(result), self.wFinal)
        return nn.AddBias(fin, self.b3)
    ...
    def train(self, dataset):
        ...
        while True:
            for x, y in dataset.iterate_once(2):
                gradient = nn.gradients(self.get_loss(x, y), [self.w, self.wHidden, self.wFinal, self.b1, self.b2, self.b3])
